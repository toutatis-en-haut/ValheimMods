    ...
    internal static class AssetLoader
    {
        public static Sprite TryLoadSprite(string relativePath, bool keyWhiteToAlpha = false)
        {
            try
            {
                ...
                {
                    PersonalGatewayPlugin.Log?.LogInfo($"Asset not found: {fullPath} (falling back to procedural)");
                    return null;
                }
            ...
            {
                PersonalGatewayPlugin.Log?.LogWarning($"AssetLoader.TryLoadSprite('{relativePath}') failed: {ex.Message}");
                return null;
            }
        }
